  ...
  const adminFeatures = [
    {
      ...
      iconBg: 'bg-blue-100 text-[#1854d6]',
    },
    {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-12">
        {/* Header */}
        <div className="mb-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#1854d6]/10 text-[#1854d6] text-sm font-medium mb-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {adminFeatures.map((feature) => (
            <Link key={feature.href} href={feature.href} className="group">
              <div className="relative flex items-start gap-4 p-6 rounded-2xl bg-white border border-slate-200 hover:border-[#1854d6]/30 hover:shadow-lg hover:shadow-[#1854d6]/5 transition-all duration-200">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${feature.iconBg}`}>
                  <feature.icon className="w-6 h-6" />
                </div>
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg font-semibold text-slate-900 group-hover:text-[#1854d6] transition-colors">
                    {t(feature.titleKey)}
                  </h2>
                ...
                <ArrowRight className="w-5 h-5 text-slate-300 group-hover:text-[#1854d6] group-hover:translate-x-0.5 transition-all mt-1 shrink-0" />
              </div>
            </Link>
          ))}
        </div>
