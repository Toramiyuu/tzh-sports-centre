...
export function TimetableGrid({
  ...
  return (
    <div className="relative border rounded-lg overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[800px]">
          ...
          <div className="grid grid-cols-8 border-b bg-background">
            {/* Time column header */}
            <div className="sticky left-0 z-20 bg-background border-r p-2 text-center text-sm font-medium text-muted-foreground">
              {t('time') || 'Time'}
            </div>
            {/* Day headers */}
            {weekDays.map((day) => (
              <div
                ...
                className={cn(
                  ...
                  day.isToday && 'bg-background'
                )}
              >
                <div className={cn(
                  'font-medium text-sm',
                  day.isToday ? 'text-foreground' : 'text-foreground'
                )}>
          ...
          <div className="grid grid-cols-8">
            {timeSlots.map((slot) => (
              <Fragment key={slot.time}>
                {/* Time column */}
                <div className="sticky left-0 z-10 bg-white border-r border-b p-1 text-xs text-muted-foreground text-right pr-2 flex items-center justify-end min-h-[40px]">
                  {slot.displayName}
                </div>
                {/* Day columns */}
                {weekDays.map((day) => {
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-4 p-3 border-t bg-background text-xs">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-4 h-4 rounded bg-secondary border border-border" />
          <span className="text-muted-foreground">{t('myLesson')}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-4 h-4 rounded bg-yellow-100 border border-yellow-300" />
          <span className="text-muted-foreground">{t('myPending')}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-4 h-4 rounded bg-purple-100 border border-purple-300" />
          <span className="text-muted-foreground">{t('coachSuggested') || 'Coach Suggested'}</span>
        </div>
        <div className="flex items-center gap-1.5">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-background rounded-lg border">
                  <p className="text-xs text-muted-foreground mb-1">{t('suggestionDialog.yourRequest') || 'Your request'}</p>
              ...
              {selectedSuggestion.adminNotes && (
                <div className="p-3 bg-background rounded-lg border border-blue-200">
                  <p className="text-xs text-foreground mb-1">{t('suggestionDialog.coachNote') || 'Note from coach'}</p>
